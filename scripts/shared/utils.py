...
def remove_extreme_outliers(
    df: pd.DataFrame, column: str = "like_count", threshold: int = 1000
) -> pd.DataFrame:
    """
    移除極端離群值（例如萬讚爆文）
    ...
    Args:
        df: DataFrame
        column: 要檢查的欄位
        threshold: 閾值，超過此值的視為極端離群值（預設 1000 讚）
    ...
    if removed_count > 0:
        removed_posts = df[df[column] > threshold]
        max_likes = removed_posts[column].max()
        print(f"\n--- Extreme Outlier Removal ({column} > {threshold}) ---")
        print(f"Removed: {removed_count} posts ({removed_count / original_count:.1%})")
        print(f"Max removed: {max_likes:.0f} likes")
        print(f"Remaining: {len(df_clean)} posts")
    else:
        print(f"\n--- No extreme outliers found ({column} > {threshold}) ---")
